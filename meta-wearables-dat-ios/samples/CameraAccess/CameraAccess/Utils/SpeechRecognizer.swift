...
public actor SpeechRecognizer: Observable {
     ...
     */
    init(viewModel: StreamSessionViewModel) {
    ...
    nonisolated private func transcribe(_ message: String) {
        Task { @MainActor in
            await self.viewModelUpdate(message)
        }
    }

    private func viewModelUpdate(_ text: String) async -> Void{
        logger.notice("In sp:\(text)")
        await viewModel.updateTranscript(text)
    }
    nonisolated private func transcribe(_ error: Error) {
        ...
        Task { @MainActor [errorMessage] in
            await self.viewModelUpdate("<< \(errorMessage) >>")
        }
    }
}
